...
### 2026-08
- **fix**: [2026-08-12] 修复复制项目时高级/环境配置（`groupConfig`）中登录接口配置（`loginApiConfigs`）和变量提取规则（`extractRules`）关联老项目 `docId` 的问题：1. 在 `SimpleModelUtils.java` 中新增 `remapGroupConfigDocIds` 方法，解析 `groupConfig` JSON，将 `loginApiConfigs` 和 `envParams[].extractRules` 中的 `apiId` 根据复制时生成的 `docMappings` 映射关系自动更新为新项目的 `docId`；2. 在 `ApiProjectServiceImpl.copyProject` 中，文档复制完成后触发 `remapGroupConfigDocIds` 重映射并保存新 `groupConfig`，确保复制项目后的登录调试与变量提取规则自动对齐至新项目的 API 节点；3. 优化 `ApiProjectTaskServiceImpl.copyProjectTasks` 中获取映射目录时的空指针防护逻辑，提升系统稳定性。
