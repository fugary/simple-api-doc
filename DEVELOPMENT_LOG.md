...
### 2026-08
- **feat**: [2026-08-19] AI 配置测试功能全流程优化与精简去重：1. 未保存即时测试与窗口防关：AiConfigList.vue 在【新增】与【编辑】弹窗底部均展示【测试】按钮，点击前置校验 Base URL / API Key 后调起测试窗且不关闭当前编辑窗；2. 标题回显与参数精简：AiConfigTestWindow.vue 统一通过 config 参数自动解析模型、配置名称与 Provider Tag，移除冗余的 defaultModel prop；3. 移除后端多余重载与DB查重：AiConfigController.java 将 /{id}/test 与 /test 合并为支持可选 id 路径变量的统一端点；AiServiceImpl 中精简 testAiConfig 与 loadModels 逻辑，直接消费传入的完整 config 实体，移除多余的数据库二次查询与字段合并；4. 前端 ESLint 0 errors 全量通过，后端 Maven 18 tests 全部通过。
