...
### 2026-08
- **feat**: [2026-08-17] 优化文档导出、项目分享、代码生成及环境配置等选择树弹窗中“已废弃”与“已禁用/删除”状态的样式展示：1. 保持通用组件纯粹与解耦：`TreeIconLabel.vue` 与 `TreeCheckConfig.vue` 保持为纯粹通用的树形展示与选择组件，不侵入任何 API 业务字段与硬编码逻辑；2. 业务选择树统一规范呈现：在 `ApiProjectShares.vue`（项目分享）、`ApiDocExportWindow.vue`（文档导出）、`ApiDocCodeGenWindow.vue`（客户端代码生成）与 `ApiEnvParams.vue`（环境配置接口选择）等业务弹窗中，统一与左侧主导航树对齐，使用 `<el-text :type="!data.enabled ? 'danger' : (data.deprecated ? 'warning' : '')" class="custom-tree-node">` 以及 `<del v-if="data.deprecated">` 渲染树节点，使废弃接口（黄色警示+删除线）与禁用接口（红色危险色）在所有选择树弹窗中表现规范统一；3. 前端全量通过 ESLint 校验。
