...
### 2026-08
- **feat**: [2026-08-11] 参考 `simple-boot-mock-server07` 在 API 项目页面新增“我的数据”筛选功能：1. 后端在 `ProjectQueryVo` 中新增 `onlyMine` 字段，并在 `ApiProjectController.java` 的 `search` 方法中增加 `onlyMine` 条件过滤：开启 `onlyMine` 时按 `user_name = loginUserName` 过滤仅展示当前登录用户创建的项目（支持叠加 `groupCode` 筛选）；2. 前端复用已有 `api.label.dashboardMyData` 词条，在 `GlobalConstants.js` 中新增 `useSearchOnlyMine` 表单配置；在 `ApiProjects.vue`（API项目）搜索栏最左侧加入“我的数据”开关控制，当开启时自动禁用用户下拉选择框，并动态过滤“项目分组”下拉选项仅保留当前登录用户创建的分组（若开启时已选的分组非本人创建则自动重置清空）；3. 后端单元测试及前端 ESLint 校验全量通过。
