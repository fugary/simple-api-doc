...
### 2026-08
- **feat**: [2026-08-11] 参考 `simple-boot-mock-server07` 在 API 项目、导入管理及分享管理页面新增“我的数据”筛选功能：1. 后端在 `ProjectQueryVo` 中新增 `onlyMine` 字段，并在 `ApiProjectController.java`、`ApiProjectTaskController.java` 和 `ApiProjectShareController.java` 的 `search` 方法中增加 `onlyMine` 条件过滤：开启 `onlyMine` 时过滤仅展示当前登录用户创建的项目/导入任务/分享链接（支持叠加 `groupCode` 筛选）；2. 前端复用已有 `api.label.dashboardMyData` 词条，在 `GlobalConstants.js` 中新增 `useSearchOnlyMine` 表单配置；在 API项目（`ApiProjects.vue`）、导入管理（`ApiProjectTasks.vue`）和分享管理（`ApiProjectShares.vue`）搜索栏最左侧加入“我的数据”开关控制，当开启时自动禁用用户下拉选择框，并动态联动过滤“项目分组”及“项目”下拉选项仅保留当前登录用户创建的分组与项目（若开启时已选的分组/项目非本人创建则自动重置清空）；同时对于从具体项目详情内嵌入进入的导入任务与分享列表（`inProject = true`）自动禁用/隐藏该筛选控件；3. 后端单元测试及前端 ESLint 校验全量通过。
