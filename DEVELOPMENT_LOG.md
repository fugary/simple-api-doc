...
### 2026-07
- **feat**: [2026-07-22] 优化项目安全认证默认值配置窗口，修复表格行按钮 `labelKey` (使用已有的 `api.label.defaultValue`)，独立配置弹窗标题为【编辑默认值】，完全复用详情调试页统一的 `<ApiRequestFormAuthorization>` 多 Tab 交互面板与 `$JWT_TOKEN` 便捷输入体验。
